...
const HeroBanner = () => {
  ...
  return (
    <section className="relative h-[70vh] lg:h-[80vh] overflow-hidden">
      <Carousel className="h-full" opts={{ loop: true, direction: isArabic ? 'rtl' : 'ltr' }} setApi={setApi}>
        <CarouselContent className="h-full">
          {slides.map((slide, idx) => (
            <CarouselItem key={idx} className="h-full">
              <div className="relative h-full min-h-[70vh] lg:min-h-[80vh]">
                <img 
                  src={slide.src} 
                ...
                <div className="absolute inset-0 px-8">
                  <div className={`absolute bottom-6 ${isArabic ? 'left-6' : 'right-6'}`}>
                    <Link to={'/shop'}>
                      <Button className="px-3 py-2 text-sm bg-transparent border border-white text-white hover:bg-white/10">
                        {t(slide.descKey)} <ArrowRight className={`ml-2 h-4 w-4 ${isArabic ? 'rotate-180' : ''}`} />
                      </Button>
                    </Link>
                  </div>
                </div>
